...
func _Shoot():
	...
	for id in pending_bullets_id.size():
		BulletMap.TouchBulletData(pending_bullets_id[id], BulletMap.bullet_data.SPEED, true, _speed_step * temp_stream_index)
...
func RePooling(): #This can be run many times to change some important settings
	var temp_settings : bool = _allow_shooting
